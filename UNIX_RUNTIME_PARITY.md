...
| Native serial device | Partial | Unix defines `SERIAL_PORT`; `od-unix/serial.cpp` supports POSIX termios devices with CTS/RTS and modem-status handling. | Test with a real device and add documented examples. |
| TCP/telnet serial listener | Partial | `od-unix/serial.cpp` supports `TCP:host:port`, `TCP://host:port`, and `/wait` with basic telnet IAC filtering. | Add tests and consider pseudo-terminal convenience endpoints. |
...
| Clipboard | Partial | `od-unix/clipboard.cpp` implements host text paste-to-keyboard through `keybuf_inject`, using `pbpaste` on macOS and `wl-paste`/`xclip`/`xsel` on Linux. The Amiga clipboard-device hooks still return no data, and the Qt Clipboard sharing option is disabled. | Add a native full clipboard sharing backend before enabling the Clipboard sharing UI control. |
...
| Config path and Unix path expansion | Enabled/Partial | `od-unix/config.cpp` and Qt expand `~/`, `$VAR`, and `${VAR}`. `od-unix/path_expand_test.cpp` covers the core helper. `~user` is documented as unsupported; relative-path policy is still open. | Add Qt boundary coverage and define relative-path base directories consistently. |
...
| macOS app and DMG packaging | Partial | CMake targets `winuae_unix_macos_app` and `winuae_unix_macos_dmg` run the packaging scripts. | Add signing/notarization and test older deployment targets with matching dependencies. |
