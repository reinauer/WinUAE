...
| Windows feature area | Unix status | Evidence | Next action |
| --- | --- | --- | --- |
| Threads, timers, memory mapping, dynamic loading | Enabled | CMake links `od-unix/threading.cpp`, `od-unix/time.cpp`, `od-unix/mman.cpp`, and shared `dlopen.cpp`; Unix defines `SUPPORT_THREADS`. CMake exposes the Unix smoke scripts as `winuae_unix_smoke_*` targets for repeatable local/CI runs. | Keep covered by normal boot and filesystem tests. |
