...
| Sample ripper | Partial | The core `audio_sampleripper()` path is linked on Unix and the integrated Qt Output page now toggles `sampleripper_enabled` like the Windows Output dialog. Ripped WAV files use `fetch_ripperpath()`. | Validate with a real sample-ripping workload and make sure the Rips directory/path behavior matches the Paths page. |
| Pro Wizard module ripper | Partial | CMake option `WINUAE_UNIX_WITH_PROWIZARD` defaults on, defines `PROWIZARD`, compiles the same Pro Wizard source list as the Windows project, and enables the integrated runtime Qt Output-page button. Unix currently uses a minimal non-interactive `gui_message_multibutton()` fallback that logs prompts and auto-accepts saves. | Replace the prompt fallback with a Qt/user-visible confirmation path and validate real module-ripping output. |
