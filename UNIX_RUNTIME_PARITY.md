...
| Windows feature area | Unix status | Evidence | Next action |
| --- | --- | --- | --- |
| Threads, timers, memory mapping, dynamic loading | Enabled | CMake links `od-unix/threading.cpp`, `od-unix/time.cpp`, `od-unix/mman.cpp`, and shared `dlopen.cpp`; Unix defines `SUPPORT_THREADS`. Unix semaphores now follow the Windows event model used by the core: posts coalesce, auto-reset waits consume one signal, manual-reset waits remain signaled, `uae_sem_unpost()` clears the event, and try-wait returns `0` on success / negative on timeout. `winuae_unix_threading_test` covers those host semantics, and CMake exposes the Unix smoke scripts as `winuae_unix_smoke_*` targets for repeatable local/CI runs. | Keep covered by normal boot, filesystem, and host primitive tests. |
